...
export function SettingsPage({ onUnsavedChangesChange, onResetRequest, }: SettingsPageProps) {
    ...
    const handleSave = async () => {
        const pathChanged = tempSettings.downloadPath !== savedSettings.downloadPath;
        ...
        // Music folder = library folder: register + scan the new location.
        if (pathChanged && persistedSettings.downloadPath) {
            try {
                const added = await EnsureLibraryFolder(persistedSettings.downloadPath);
                if (added)
                    toast.info("Music folder added to your library — scanning in the background");
            }
            catch { /* library not initialised yet */ }
        }
    };
      ...
      <div className="flex-1 overflow-y-auto pt-4">
            ...
            <div className="space-y-4 lg:pr-8 lg:border-r">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground pt-1">
                  Always downloads the best quality each source offers, falling back automatically when a track's top tier isn't available. The community backend is rate-limited to 1 track every 30 seconds.
                </p>
              </div>
            </div>
        ...
        {activeTab === "metadata" && (<div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <Switch id="auto-download-lyrics" checked={tempSettings.autoDownloadLyrics !== false} onCheckedChange={(checked) => setTempSettings((prev) => ({
                ...prev,
                autoDownloadLyrics: checked,
            }))}/>
                <Label htmlFor="auto-download-lyrics" className="cursor-pointer text-sm font-normal">
                  Auto-download Lyrics (.lrc next to each track)
                </Label>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="lastfm-api-key" className="text-sm">Last.fm API Key</Label>
                <Input id="lastfm-api-key" type="password" autoComplete="off" placeholder="Optional — fills artist bios Spotify doesn't have"
                    value={tempSettings.lastfmApiKey} onChange={(e) => setTempSettings((prev) => ({
                    ...prev,
                    lastfmApiKey: e.target.value.trim(),
                }))}/>
                <p className="text-xs text-muted-foreground">
                  Used as a fallback for artist bios during library enrichment (photos fall back to Deezer automatically, no key needed).
                </p>
              </div>
