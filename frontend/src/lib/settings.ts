...
function normalizeSettingsPayload(settings: SettingsPayload): SettingsPayload {
    ...
    // Per-service quality keys were removed: downloads always use the best
    // available quality (enforced backend-side).
    delete normalized.tidalQuality;
    delete normalized.qobuzQuality;
    delete normalized.amazonQuality;
    delete normalized.autoQuality;
...
export async function loadSettings(): Promise<Settings> {
    try {
        ...
        if (backendSettings) {
            ...
            // config.json is the single source of truth; localStorage is only
            // a boot cache (instant theme/font before this async load lands).
            // Mirror the authoritative copy so the cache can never drift.
            try { localStorage.setItem(SETTINGS_KEY, JSON.stringify(cachedSettings)); } catch { /* ignore */ }
