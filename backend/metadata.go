...
func ExtractCoverArt(filePath string) (string, error) {
	...
	if err != nil || coverPath == "" {
		Dbgf("[ExtractCoverArt] Library extraction failed for %s, trying FFmpeg fallback...\n", filePath)
...
func ExtractLyrics(filePath string) (string, error) {
	...
	if (err != nil || lyrics == "") && ext != ".m4a" {
		Dbgf("[ExtractLyrics] Library extraction failed for %s, trying ffprobe fallback...\n", filePath)
...
func extractLyricsFromMp3(filePath string) (string, error) {
	...
	if len(usltFrames) == 0 {
		Dbgf("[ExtractLyrics] No USLT frames found in MP3: %s\n", filePath)
		return "", nil
	}

	uslt, ok := usltFrames[0].(id3v2.UnsynchronisedLyricsFrame)
	if !ok {
		Dbgf("[ExtractLyrics] USLT frame type assertion failed in MP3: %s\n", filePath)
		return "", nil
	}

	if uslt.Lyrics == "" {
		Dbgf("[ExtractLyrics] USLT frame has empty lyrics in MP3: %s\n", filePath)
		return "", nil
	}

	Dbgf("[ExtractLyrics] Successfully extracted lyrics from MP3: %s (%d characters)\n", filePath, len(uslt.Lyrics))
...
func extractLyricsFromFlac(filePath string) (string, error) {
			...
			for _, comment := range cmt.Comments {
				...
				if len(parts) == 2 {
					...
					if fieldName == "LYRICS" || fieldName == "UNSYNCEDLYRICS" {
						lyrics := parts[1]
						Dbgf("[ExtractLyrics] Successfully extracted lyrics from FLAC: %s (%d characters)\n", filePath, len(lyrics))
						return lyrics, nil
					}
				}
			}
	...
	Dbgf("[ExtractLyrics] No lyrics found in FLAC: %s\n", filePath)
	return "", nil
}
...
func EmbedLyricsOnlyMP3(filepath string, lyrics string) error {
	...
	if err != nil {
		Dbgf("[EmbedLyricsOnlyMP3] Warning: Failed to validate lyrics duration: %v, using original lyrics\n", err)
...
func embedLyricsToM4A(filepath string, lyrics string) error {
	...
	validatedLyrics, err := validateLyricsDuration(lyrics, filepath)
	if err != nil {
		Dbgf("[embedLyricsToM4A] Warning: Failed to validate lyrics duration: %v, using original lyrics\n", err)
		validatedLyrics = lyrics
	}
	lyrics = validatedLyrics
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		Dbgf("[FFmpeg] Error embedding lyrics to M4A: %s\n", string(output))
	...
	Dbgf("[FFmpeg] Lyrics embedded to M4A successfully: %d characters\n", len(lyrics))
	return nil
}
...
func EmbedLyricsOnlyUniversal(filepath string, lyrics string) error {
	...
	validatedLyrics, err := validateLyricsDuration(lyrics, filepath)
	if err != nil {
		Dbgf("[EmbedLyricsOnlyUniversal] Warning: Failed to validate lyrics duration: %v, using original lyrics\n", err)
		validatedLyrics = lyrics
	}
	lyrics = validatedLyrics
...
func validateLyricsDuration(lyrics string, filepath string) (string, error) {
	...
	if err != nil {
		...
		Dbgf("[ValidateLyrics] Warning: Could not get audio duration: %v, skipping validation\n", err)
		return lyrics, nil
	}

	if duration <= 0 {

		Dbgf("[ValidateLyrics] Warning: Invalid duration (%f seconds), skipping validation\n", duration)
		return lyrics, nil
	}
			...
			if closeBracket > 0 {
				...
				if ms >= 0 {
					if ms <= durationMs {
						...
						Dbgf("[ValidateLyrics] Filtered out line with timestamp %s (exceeds duration %d ms): %s\n", timestampStr, durationMs, trimmedLine)
					}
				} else {
