...
func FilterSearch(data map[string]interface{}, separator string) map[string]interface{} {
	...
	// Podcasts (shows) and audiobooks ride along in the same searchDesktop
	// payload — the URI's type segment drives the open.spotify.com URL.
	extractShowLike := func(sectionKey string) {
		sectionData := getMap(searchData, sectionKey)
		items := getSlice(sectionData, "items")
		if items == nil {
			return
		}
		for _, item := range items {
			...
			coverObj := extractCoverImage(getMap(entry, "coverArt"))
			var cover interface{}
			if coverObj != nil {
				cover = getString(coverObj, "medium")
			}
			// Podcasts credit a publisher; audiobooks credit authors.
			byline := getString(getMap(entry, "publisher"), "name")
			if byline == "" {
				byline = getString(entry, "publisherName")
			}
			if byline == "" {
				if authors := getSlice(entry, "authors"); authors != nil {
					names := []string{}
					...
					byline = strings.Join(names, separator)
				}
			}
			url := ""
			if urlType != "" {
				url = fmt.Sprintf("https://open.spotify.com/%s/%s", urlType, id)
			}
			section := results[sectionKey].([]map[string]interface{})
			results[sectionKey] = append(section, map[string]interface{}{
				"id":     id,
				"name":   name,
				"byline": byline,
				"cover":  cover,
	...
	extractShowLike("podcasts")
	extractShowLike("audiobooks")
	...
	podcasts := results["podcasts"].([]map[string]interface{})
	audiobooks := results["audiobooks"].([]map[string]interface{})
