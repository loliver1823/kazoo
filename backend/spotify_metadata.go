...
type SearchResponse struct {
	...
	Audiobooks []SearchResult `json:"audiobooks"`
}
...
func (c *SpotifyMetadataClient) formatArtistDiscographyData(ctx context.Context, raw *apiArtistResponse, callback MetadataCallback) (*ArtistDiscographyPayload, error) {
	...
	for _, alb := range raw.Discography.All {
		go func(albumID string, albumName string) {
			...
			if err != nil {
				Dbgf("Error getting tracks for album %s: %v\n", albumName, err)
...
func (c *SpotifyMetadataClient) Search(ctx context.Context, query string, limit int) (*SearchResponse, error) {
	...
	response := &SearchResponse{
		...
		Podcasts:   make([]SearchResult, 0),
		Audiobooks: make([]SearchResult, 0),
	}
	...
	for _, item := range apiResp.Results.Podcasts {
		url := item.URL
		if url == "" {
			url = fmt.Sprintf("https://open.spotify.com/show/%s", item.ID)
		}
		response.Podcasts = append(response.Podcasts, SearchResult{
			ID:          item.ID,
			Name:        item.Name,
			Type:        "podcast",
			Owner:       item.Byline,
	...
	for _, item := range apiResp.Results.Audiobooks {
		url := item.URL
		if url == "" {
			url = fmt.Sprintf("https://open.spotify.com/show/%s", item.ID)
		}
		response.Audiobooks = append(response.Audiobooks, SearchResult{
			ID:          item.ID,
			Name:        item.Name,
			Type:        "audiobook",
			Owner:       item.Byline,
